...
# from fractional_neural_sde.pathCDE import NeuralCDE
...
class LatentSDE(BaseSDE):
    def __init__(
        ...
        # Other paths: FIM, which would just be the noise term (can be ito integrated), and a control eq (RNN equivalent)
        ...
        self.CDE_path = torch.zeros(noise_path.num_steps+1)
    ...
    def drift_pass(self, y0):
        ...
        Z = self.noise_path.Z
        ts = torch.cat([Z.clone(), (Z[num_inducings-2:num_inducings-1]+dt)], dim = -1)
        drift_path = y0.reshape(1)
        t_prev = 0
        y_n = drift_path
        for tn in ts:
            dt = tn - t_prev
            dY = self.net(torch.tensor([torch.sin(tn), torch.cos(tn), y_n]))
            #dY = self.h(torch.tensor(tn), y_n)
            y_n = (y_n + dY*dt)
            t_prev = tn
            drift_path = torch.cat([drift_path, y_n]) # This treats this like an ODE euler method solver 
        return ts, drift_path[1:drift_path.shape[0]] + 1e-5 # (small offset)
        
    def precompute_drift(self, y0):
        # sample initial point
        # get time series to "integrate" over
        ts, drift_path = self.drift_pass(y0)
        self.CDE_path = torch.stack([ts, drift_path], dim = -1)

    def precompute_white_noise(self, batch_size, ts, y0):
        """Precompute Cholesky for white noise"""
        self.precompute_drift(y0 = y0)
        self.noise_path.initialize_path(self.CDE_path)
        self.noise_path.precompute(batch_size = batch_size)
    ...
    def sample_q(self, ts, batch_size, sdeint_fn, method, dt, bm, eps: None):
        ...
        self.precompute_white_noise(batch_size=batch_size, ts = ts, y0 = y0)

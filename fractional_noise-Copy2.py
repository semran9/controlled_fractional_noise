...
class SparseGPNoise(nn.Module):
    ...
    def increment_weight(self, t, yt, y0):
        r"""
        ...
        Args:
            t: input time
            ys: prior weight, new weight?
        """

        ta, tb = t, t + self.dt
        ya, yb = y0, yt # subsetting
        wa, wb = self.compute_weight(ta, ya).clamp(min=0), self.compute_weight(tb, yb).clamp(
            min=0
        )
    ...
    def compute_weight(self, t, ys = None, eps=1e-12):
    ...
    def precompute(self, y0 = None, yt = None, batch_size: int = 1):
        """
        ...
        Kz = self.increment_weight(self.Z, yt, y0)  # num_step, num_inducing, dim
    ...
    def compute_hurst(self, t, ys = None):
        """Compute Hurst function"""
        t = t.view(-1, 1)
        # time is converted into a positional encoding
        out = self.driving_action(torch.cat([torch.sin(t), torch.cos(t), t, ys], dim=-1))
        return out

    def forward(self, t, yt = None, y0 = None):
        """
        ...
        * So, what we need to do is make the current framework work with single numbers
        ...
        Kt = self.increment_weight(t, yt, y0)  # num_step, 1, dim
    ...
    @torch.no_grad()
    def sample_alternative(self, size, y0 = None):
        """Sample paths"""

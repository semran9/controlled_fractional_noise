...
class LatentSDE(BaseSDE):
    def __init__(
        ...
        self.CDE = NeuralCDE(input_channels=3, hidden_channels=10, output_channels=1)
    ...
    def drift_pass(self, y0):
        ...
        ts = torch.linspace(t0 + dt * 5.5, t1 - dt * 4.5, num_inducings+1)
        ...
        return drift_path[1:drift_path.shape[0]] + 1e-5# (small offset)
    ...
    def precompute_CDE(self, y0):
        # precompute CDE using pathCDE

    def precompute_white_noise(self, batch_size, ts, y0):
        """Precompute Cholesky for white noise"""
        self.precompute_drift(y0 = y0)
        self.noise_path.precompute(batch_size = batch_size, y0 = self.drift_y0, 
                                   yt = self.drift_yt)

...
class LatentSDE(BaseSDE):
    def __init__(
        ...
        self.qy0_mean = nn.Parameter(torch.tensor([[self.mu]]), requires_grad=True)
    ...
    def precompute_drift(self, y0):
        ...
        for tn in ts:
            y_n = drift_path[count]
            dY = self.net(torch.tensor([torch.sin(tn), torch.cos(tn), y0]))
            drift_path.append((y_n+dY)) # This treats this like an ODE euler method solver 
            count+=1
        drift_path = torch.tensor(drift_path).reshape(-1,1)
    ...
    def precompute_white_noise(self, batch_size, ts, y0):
        ...
        self.noise_path.precompute(batch_size = batch_size, y0 = self.drift_y0, yt = self.drift_yt)
    ...
    def sample_q(self, ts, batch_size, sdeint_fn, method, dt, bm, eps: None):
        ...
        self.precompute_white_noise(batch_size=batch_size, ts = ts, y0 = y0.data.detach())
